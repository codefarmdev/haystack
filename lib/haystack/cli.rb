    ...
    class << self
      ...
      def run(argv=ARGV)
        ...
        if command
          if AVAILABLE_COMMANDS.include?(command)
            ...
            puts "Command '#{command}' does not exist, run haystack -h to "\
              "see the help"
            exit(1)
          end
        else
      ...
      def global_option_parser
        OptionParser.new do |o|
          o.banner = 'Usage: haystack <command> [options]'

          o.on '-v', '--version', "Print version and exit" do |arg|
            puts "Haystack #{Haystack::VERSION}"
            exit(0)
          end
      ...
      def command_option_parser
        {
          'notify_of_deploy' => OptionParser.new do |o|
            o.banner = 'Usage: haystack notify_of_deploy [options]'
