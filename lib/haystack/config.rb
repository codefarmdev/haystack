...
module Haystack
  class Config
    ...
    def initialize(root_path, env, initial_config={}, logger=Haystack.logger)
      @root_path      = root_path
      @env            = env.to_s
      ...
      elsif ENV['HAYSTACK_PUSH_API_KEY']
        load_default_config_with_push_api_key_and_name_from_env(
          ENV['HAYSTACK_PUSH_API_KEY']
        )
      elsif ENV['HAYSTACK_API_KEY']
        load_default_config_with_push_api_key_and_name_from_env(
          ENV['HAYSTACK_API_KEY']
        )
        @logger.info(
          'The HAYSTACK_API_KEY environment variable has been deprecated, ' \
          'please switch to HAYSTACK_PUSH_API_KEY'
        )
      else
        carefully_log_error(
      ...
      if config_hash && !config_hash[:name]
        @logger.debug(
          "There's no application name set in your config file or in the HAYSTACK_APP_NAME env var. " \
    ...
    def config_file
      @config_file ||= File.join(root_path, 'config', 'haystack.yml')
    end
    ...
    def load_default_config_with_push_api_key_and_name_from_env(key)
      ...
      @config_hash[:name]   = ENV['HAYSTACK_APP_NAME'] if ENV['HAYSTACK_APP_NAME']
      @config_hash[:active] = ENV['HAYSTACK_ACTIVE'] == 'true' if ENV['HAYSTACK_ACTIVE']
    end
