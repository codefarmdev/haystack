...
describe "Resque integration" do
  let(:file) { File.expand_path('lib/haystack/integrations/resque.rb') }
  ...
  context "with resque" do
    ...
    describe :around_perform_resque_plugin do
      let(:transaction) { Haystack::Transaction.new(1, {}) }
      let(:job) { Resque::Job }
      let(:invoked_job) { nil }
      before do
      ...
      context "without exception" do
        it "should create a new transaction" do
          Haystack::Transaction.should_receive(:create).and_return(transaction)
        end
