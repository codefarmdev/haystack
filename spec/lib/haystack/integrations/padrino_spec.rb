...
if padrino_present?
  describe "Padrino integration"   do
    require File.expand_path('lib/haystack/integrations/padrino.rb')
    ...
    describe "Haystack::Integrations::PadrinoPlugin" do
      before do
        Haystack.stub(
          :active?      => true,
          :start        => true,
      ...
      it "should start the logger on init" do
        expect( Haystack ).to receive(:start_logger)
      end

      it "should start haystack on init" do
        expect( Haystack ).to receive(:start)
      end

      it "should add the Listener middleware to the stack" do
        expect( Padrino ).to receive(:use).with(Haystack::Rack::Listener)
      end
      ...
      after { Haystack::Integrations::PadrinoPlugin.init }
    end
      ...
      describe "#route!" do
        ...
        context "when Sinatra tells us it's a static file" do
          ...
          it "should call the original method" do
            expect( router ).to receive(:route_without_haystack)
          end
        ...
        context "with an exception" do
          before { router.stub(:route_without_haystack).and_raise(VerySpecificError) }

          it "should add the exception to the current transaction" do
            expect( Haystack ).to receive(:add_exception)
