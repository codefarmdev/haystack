...
describe "Sidekiq integration" do
  let(:file) { File.expand_path('lib/haystack/integrations/sidekiq.rb') }
  before :all do
    module Sidekiq
  ...
  let(:worker) { double }
  let(:queue) { double }
  let(:current_transaction) { Haystack::Transaction.create(SecureRandom.uuid, {}) }
  let(:item) {{
    'class'       => 'TestClass',
    'retry_count' => 0,
    'queue'       => 'default',
  ...
  let(:plugin) { Haystack::Integrations::SidekiqPlugin.new }

  before do
    Haystack.stub(:is_ignored_exception? => false)
    Haystack::Transaction.stub(:current => current_transaction)
  end
  ...
  context "with a performance call" do
    ...
    after do
      Timecop.freeze(Time.parse('01-01-2001 10:01:00UTC')) do
        Haystack::Integrations::SidekiqPlugin.new.call(worker, item, queue) do
          # nothing
        end
      end
    end
  ...
  context "with an erroring call" do
    ...
    it "should add the exception to haystack" do
