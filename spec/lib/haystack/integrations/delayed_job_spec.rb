...
describe "Delayed Job integration" do
  let(:file) { File.expand_path('lib/haystack/integrations/delayed_job.rb') }
  ...
  context "with delayed job" do
    ...
    describe ".invoke_with_instrumentation" do
      let(:plugin) { Haystack::Integrations::DelayedPlugin }
      ...
      context "with a normal call" do
        it "should wrap in a transaction with the correct params" do
          Haystack.should_receive(:monitor_transaction).with(
        ...
        context "with custom name call" do
          let(:job) do
            double(
              :payload_object => double(
                :haystack_name => 'CustomClass#perform'
              ),
              :id         => 123,
              :name       => 'TestClass#perform',
              :priority   => 1,
      ...
      context "with an erroring call" do
        it "should add the error to the transaction" do
          Haystack::Transaction.any_instance.should_receive(:add_exception).with(error)
          invoked_block.stub(:call).and_raise(error)
          Haystack::Transaction.any_instance.should_receive(:complete!)
    ...
    it "should add the plugin" do
      ::Delayed::Worker.plugins.should include Haystack::Integrations::DelayedPlugin
    end
  end
