...
describe Haystack::Transaction do
  ...
  describe '.create' do
    subject { Haystack::Transaction.create('1', {}) }

    it 'should add the request id to the thread local' do
      subject
      Thread.current[:haystack_transaction_id].should == '1'
    end

    it "should create a transaction" do
      subject.should be_a Haystack::Transaction
      subject.request_id.should == '1'
    end
  end

  describe '.current' do
    let(:transaction) { Haystack::Transaction.create('1', {}) }
    before { transaction }
    subject { Haystack::Transaction.current }
  ...
  describe "complete_current!" do
    before { Thread.current[:haystack_transaction_id] = nil }

    context "with a current transaction" do
      before { Haystack::Transaction.create('2', {}) }

      it "should complete the current transaction and reset the thread haystack_transaction_id" do
        Haystack::Transaction.current.should_receive(:complete!)

        Haystack::Transaction.complete_current!

        Thread.current[:haystack_transaction_id].should be_nil
      end
    end
  ...
  context "with transaction instance" do
    ...
    let(:transaction) { Haystack::Transaction.create('3', env) }

    it "should add the transaction to the list" do
      transaction
      Haystack.transactions['3'].should == transaction
    end
    ...
    describe "#convert_values_to_primitives!" do
      ...
      context "without values that need to be converted" do
        ...
        it "should not covert to primitives twice" do
          ...
          Haystack::ParamsSanitizer.should_not_receive(:sanitize!)
          transaction.convert_values_to_primitives!
        end
      end
    end
    ...
    describe '#complete!' do
      ...
      it 'should remove transaction from the list' do
        expect { transaction.complete! }.
          to change(Haystack.transactions, :length).by(-1)
      end

      context 'enqueueing' do
        context 'sanity check' do
          specify { Haystack.should respond_to(:enqueue) }
        end

        context 'without events and without exception' do
          it 'should add transaction to the agent' do
            Haystack.should_receive(:enqueue).with(transaction)
          end
        end
      ...
      context 'when using IPC' do
        ...
        it "should convert to primitves and send itself trough the pipe" do
          ...
          Haystack::IPC::Client.should_receive(:enqueue).with(transaction)
        end
    ...
    describe '#sanitize_environment!' do
      let(:whitelisted_keys) { Haystack::Transaction::ENV_METHODS }
      let(:transaction) { Haystack::Transaction.create('1', env) }
      let(:env) do
    ...
    describe '#sanitize_session_data!' do
      ...
      it "passes the session data into the params sanitizer" do
        Haystack::ParamsSanitizer.should_receive(:sanitize).with({:foo => :bar}).
      ...
      context "when skipping session data" do
        before do
          Haystack.config = {:skip_session_data => true}
        end
    ...
    describe '#sanitize_params!' do
      let(:params) { {:foo => 'bar'} }
      let(:transaction) do
        Haystack::Transaction.create('1', {}, :params => params)
      end
      before  { Haystack.config = {:send_params => true} }
      subject { transaction.sanitized_params }

      it "should call the params sanitizer and set sanitized params" do
        Haystack::ParamsSanitizer.should_receive(:sanitize)
          .with(params)
          .and_return({'foo' => 'bar'})
      ...
      context "when skipping session data" do
        before { Haystack.config = {:send_params => false} }
