...
describe Haystack::Config do
  ...
  describe "when there is a config file" do
    ...
    it "should not log an error" do
      Haystack::Config.any_instance.should_not_receive(:carefully_log_error)
      subject
    end
    ...
    it "should merge with the default config and fill the config hash" do
      subject.config_hash.should == {
        ...
        :send_params                    => true,
        :endpoint                       => 'https://push.haystack.com/1',
        :slow_request_threshold         => 200,
        :push_api_key                   => 'abc',
        :name                           => 'TestApp',
        :active                         => true,
        :enable_frontend_error_catching => false,
        :frontend_error_catching_path   => '/haystack_error_catcher'
      }
    end
    ...
    context "and there's also an env var present" do
      before do
        ENV['HAYSTACK_PUSH_API_KEY'] = 'push_api_key'
      end
  ...
  context "when there is a config file without the current env" do
    ...
    it "should log an error" do
      Haystack::Config.any_instance.should_receive(:carefully_log_error).with(
  ...
  context "when there is no config file" do
    let(:initial_config) { {} }
    let(:config) { Haystack::Config.new('/nothing', 'production', initial_config) }

    it "should log an error" do
      Haystack::Config.any_instance.should_receive(:carefully_log_error).with(
        "Not loading: No config file found at '/nothing/config/haystack.yml' " \
        "and no HAYSTACK_PUSH_API_KEY env var present"
      )
      subject
    end
    ...
    context "and an env var is present" do
      ...
      it "should merge with the default config, fill the config hash and log about the missing name" do
        Haystack.logger.should_receive(:debug).with(
          "There's no application name set in your config file or in the HAYSTACK_APP_NAME env var. You should set one unless your app runs on Heroku."
        )
      ...
      context "and an initial config with a name is present" do
        ...
        it "should merge with the config" do
          Haystack.logger.should_not_receive(:debug)
      ...
      context "more config in environment" do
        context "HAYSTACK_APP_NAME is set" do
          before do
            ENV['HAYSTACK_APP_NAME'] = 'Name from env'
          end
        ...
        context "HAYSTACK_ACTIVE" do
          context "not present" do
            before do
              ENV.delete('HAYSTACK_ACTIVE')
            end
      ...
      context "with only HAYSTACK_API_KEY" do
        before do
          ENV.delete('HAYSTACK_PUSH_API_KEY')
          ENV['HAYSTACK_API_KEY'] = 'old_style_api_key'
        end
      ...
      context "with both HAYSTACK_PUSH_API_KEY and HAYSTACK_API_KEY" do
        before do
          ENV['HAYSTACK_API_KEY'] = 'old_style_api_key'
        end
