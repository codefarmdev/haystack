...
describe Haystack::Agent do
  ...
  describe "#sleep_time" do
    subject { Haystack::Agent.new.sleep_time }

    it { should == 60 }

    context "for development" do
      before do
        Haystack.config.stub(:env => 'development')
      end
  ...
  describe "#start_thread" do
    ...
    context "when an exception occurs in the thread" do
      ...
      it "should log the error" do
        Haystack.logger.should_receive(:error).
          with(kind_of(String)).
          once
  ...
  describe "#subscribe" do
    it "should have set the haystack subscriber" do
    ...
    context "handling events" do
      before :each do
        ...
        Haystack.agent.subscribe

        Haystack::Transaction.create('123', {})
      end

      it "should should not listen to events that start with a bang" do
        Haystack::Transaction.current.should_not_receive(:add_event)
      ...
      it "should add a normal event" do
        Haystack::Transaction.current.should_not_receive(:set_process_action_event)
        Haystack::Transaction.current.should_receive(:add_event).with(
      ...
      it "should add and set a process action event" do
        Haystack::Transaction.current.should_receive(:set_process_action_event).with(
          kind_of(ActiveSupport::Notifications::Event)
        ).at_least(:once)
        Haystack::Transaction.current.should_receive(:add_event).with(
          kind_of(ActiveSupport::Notifications::Event)
      ...
      it "should add and set a perform job event" do
        Haystack::Transaction.current.should_receive(:set_perform_job_event).with(
          kind_of(ActiveSupport::Notifications::Event)
        ).at_least(:once)
        Haystack::Transaction.current.should_receive(:add_event).with(
          kind_of(ActiveSupport::Notifications::Event)
    ...
    describe "#unsubscribe" do
      before :each do
        Haystack.agent.unsubscribe
      end

      it "should not have a subscriber" do
        Haystack.agent.subscriber.should be_nil
      end
  ...
  describe "#enqueue" do
    ...
    context "with ignored action" do
      before { Haystack.stub(:is_ignored_action? => true) }
  ...
  describe "#forked!" do
    ...
    it "should set active to true, create a new aggregator, set the new pid and restart the thread" do
      ...
      subject.aggregator.should be_a Haystack::Aggregator
  ...
  describe "#shutdown" do
    ...
    it "should log the reason for shutting down" do
      Haystack.logger.should_receive(:info).with('Shutting down agent (shutting down)')
    ...
    context "good responses" do
      ...
      context "with 200" do
        ...
        it "does not log the event" do
          Haystack.logger.should_not_receive(:error)
        end
      end
  ...
  context "when inactive" do
    before { Haystack.config.stub(:active? => false) }

    it "should not start a thread" do
      Thread.should_not_receive(:new)
    end

    after { Haystack::Agent.new }
  end
end
