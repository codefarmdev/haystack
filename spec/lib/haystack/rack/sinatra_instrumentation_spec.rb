...
if defined?(::Sinatra)
  describe Haystack::Rack::SinatraInstrumentation do
    before :all do
      start_agent
      @events = []
    ...
    let(:env) { {} }
    let(:middleware) { Haystack::Rack::SinatraInstrumentation.new(app, {}) }
    ...
    describe "#call" do
     before do
      ...
      context "when haystack is active" do
        before { Haystack.stub(:active? => true) }

        it "should call with monitoring" do
          expect( middleware ).to receive(:call_with_haystack_monitoring).with(env)
        end
      end
      ...
      context "when haystack is not active" do
        ...
        it "should not call with monitoring" do
          expect( middleware ).to_not receive(:call_with_haystack_monitoring)
        end
    ...
    describe "#call_with_haystack_monitoring" do
      before do
        middleware.stub(:raw_payload => {})
      ...
      it "should instrument the call" do
        ...
        middleware.call_with_haystack_monitoring(env)
      ...
      it "should add exceptions stored in env under sinatra.error" do
        ...
        Haystack.should_receive(:add_exception).with(exception)
    ...
    describe "use a custom request class and parameters method" do
      ...
      let(:middleware) { Haystack::Rack::Instrumentation.new(app, options) }
      subject { middleware.raw_payload(env) }
